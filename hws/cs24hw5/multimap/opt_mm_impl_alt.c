...
struct multimap {
    multimap_node *root;
    void *memory_bank;
    int memory_bank_num_nodes;
    int memory_bank_max_size;
};
...
void resize_values();
void resize_memory_bank(multimap *mm);
 ...
 */
multimap_node * alloc_mm_node(multimap *mm) {
    if (mm->memory_bank_num_nodes * sizeof(multimap_node) >= \
            mm->memory_bank_max_size)
        resize_memory_bank(mm);
    multimap_node *node = mm->memory_bank + mm->memory_bank_num_nodes * sizeof(multimap_node);
    mm->memory_bank_num_nodes++;
...
multimap * init_multimap() {
    ...
    mm->root = NULL;
    mm->memory_bank = NULL;
    mm->memory_bank_max_size = 0;
    mm->memory_bank_num_nodes = 0;
    return mm;
}
 ...
 */
void resize_memory_bank(multimap *mm) {
    if (mm->memory_bank_max_size == 0)
        mm->memory_bank_max_size = 100000 * sizeof(multimap_node);
    else
        mm->memory_bank_max_size *= 2;
    mm->memory_bank = realloc(mm->memory_bank, mm->memory_bank_max_size);
}
